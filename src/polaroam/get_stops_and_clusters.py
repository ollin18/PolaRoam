...
files = glob.glob(os.path.join(data_dir, "new_filtered_localized","date_trunc*"), recursive=True)
...
for date_str in unique_dates:
    ...
    df = pl.scan_parquet(os.path.join(data_dir,"new_filtered_localized", f"date_trunc={date_str}/*.parquet"))
    ...
    base_path = os.path.join(data_dir,f"new_stops_r1{model._r1}_ms{model._min_staying_time}_mt{model._max_time_between}","all_users_stops")
...
data_dir = os.path.join("/","data","Berkeley","MX","data_quadrant")
base_path = os.path.join(data_dir,f"new_stops_r1{model._r1}_ms{model._min_staying_time}_mt{model._max_time_between}")
